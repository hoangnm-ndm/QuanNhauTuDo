...
import { MenuItem } from "./menu-items.model.js";
...
export const getMenuItems = handleAsync(async (req, res) => {
  ...
  res
    .status(200)
    .json(createResponse(true, 200, "Menu items retrieved successfully", menuItems));
});

export const getMenuItemDetail = handleAsync(async (req, res) => {
  const menuItem = await MenuItem.findById(req.params.id);
  if (!menuItem) {
    return res.status(404).json(createResponse(false, 404, "Menu item not found"));
  }
  res
    .status(200)
